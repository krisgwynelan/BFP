...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .om-name-cell { font-size:15px; font-weight:700; color:#1a1714; line-height:1.25; letter-spacing:-0.01em; margin:0; white-space:nowrap; }
  .om-rank-sub  { font-size:12px; font-weight:500; color:#a8a29e; margin:2px 0 0; line-height:1.2; white-space:nowrap; }
  ...
  .om-row-action-btn { width:32px; height:32px; border-radius:7px; display:flex; align-items:center; justify-content:center; background:transparent; border:1px solid transparent; color:#9a8a84; cursor:pointer; transition:all 0.15s; }
  ...
  .om-table-scroll::-webkit-scrollbar { height: 4px; }
  .om-table-scroll::-webkit-scrollbar-track { background: #fdf9f8; }
  .om-table-scroll::-webkit-scrollbar-thumb { background: #dcc8c0; border-radius: 4px; }

  .om-table { width:max-content; min-width:100%; border-collapse:collapse; }
  .om-table th, .om-table td { vertical-align:middle; }
  .om-table th {
    text-align:left; white-space:nowrap; padding:12px 14px;
    font-size:10px; font-weight:800; text-transform:uppercase;
    letter-spacing:0.13em; color:#b0a09a; background:#fdf9f8;
  }
  .om-table td { padding:14px 14px; }
  ...
  return (
    <label className={disabled ? '' : 'om-leader-toggle'}
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <p style={{ fontSize:12, color:'#a8a29e', margin:'3px 0 0', fontWeight:500, lineHeight:1.4 }}>
...
function RowActions({ officer, onEdit, onDelete }) {
  ...
  const bRow = { width:'100%', textAlign:'left', padding:'10px 16px', fontSize:14, fontWeight:600, display:'flex', alignItems:'center', gap:8, background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' };
    ...
    <>
      ...
      {open && createPortal(
        <div ref={menuRef} className={pos.above?'om-fdup':'om-fdd'}
          ...
          <button style={{...bRow,color:'#2563eb'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(37,99,235,0.05)'} onMouseLeave={e=>e.currentTarget.style.background='none'} onClick={()=>{setOpen(false);onEdit(officer);}}>Edit Record</button>
          <button style={{...bRow,color:'#c0392b'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(192,57,43,0.05)'} onMouseLeave={e=>e.currentTarget.style.background='none'} onClick={()=>{setOpen(false);onDelete(officer);}}>Delete</button>
        </div>, document.body
      )}
    </>
  );
        ...
        <div style={{ padding:'28px 28px 24px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:12, background:'#fdf8f6', border:'1.5px solid #f0e8e5', marginBottom:20 }}>
            ...
            <div style={{ minWidth:0 }}>
              ...
              <p style={{ fontSize:12.5, color:'#a8a29e', margin:'3px 0 0', fontWeight:500 }}>{officer.rank}</p>
            </div>
          </div>
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={onCancel} disabled={deleting} style={{ flex:1, fontWeight:700, padding:'12px 0', borderRadius:11, fontSize:14, background:'#f5f0ed', border:'1px solid #e8ddd8', color:'#57534e', cursor:deleting?'not-allowed':'pointer', fontFamily:'inherit' }}>Cancel</button>
            <button onClick={onConfirm} disabled={deleting} style={{ flex:1, fontWeight:700, padding:'12px 0', borderRadius:11, fontSize:14, color:'white', border:'none', display:'flex', alignItems:'center', justifyContent:'center', gap:7, background:deleting?'#e8c4bc':'linear-gradient(135deg,#c0392b,#e74c3c)', boxShadow:deleting?'none':'0 4px 18px rgba(192,57,43,0.35)', cursor:deleting?'not-allowed':'pointer', fontFamily:'inherit' }}>
...
function FormModal({ editingOfficer, form, setForm, fileRef, uploading, saving, onSubmit, onClose }) {
  ...
  const field = { width:'100%', background:'#fafaf9', border:'1.5px solid #e8ddd8', borderRadius:10, padding:'10px 13px', fontSize:14, color:'#1a1714', outline:'none', transition:'all 0.18s', fontFamily:'inherit', lineHeight:1.5 };
      ...
      <div onClick={e=>e.stopPropagation()} style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:999991, width:'min(660px,calc(100vw - 32px))', height:'min(840px,calc(100dvh - 40px))', borderRadius:22, boxShadow:'0 40px 100px rgba(0,0,0,0.55)', background:'white', animation:'omIn 0.22s cubic-bezier(0.34,1.4,0.64,1)', display:'flex', flexDirection:'column', overflow:'hidden', fontFamily:'inherit' }}>
        ...
        <div style={{ flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px 16px', borderBottom:'1px solid #f5ede9' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              ...
              <h3 style={{ fontWeight:700, fontSize:16, color:'#1a1714', margin:0, lineHeight:1.2 }}>{editingOfficer?'Update Officer Record':'Add New Officer'}</h3>
            </div>
          </div>
            ...
            <div style={{...section}}>
              ...
              <div style={{ display:'flex', gap:14, alignItems:'center' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  ...
                  <p style={{ fontSize:11, color:'#b5a8a3', margin:0, fontWeight:500, lineHeight:1.5 }}>Any size — auto-compressed</p>
                </div>
              </div>
            </div>
            <div style={{...section}}>
            ...
            <div style={{...section}}>
              ...
              {autoLeader?(
                <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 14px', borderRadius:11, background:'rgba(192,57,43,0.04)', border:'1.5px solid rgba(192,57,43,0.2)' }}>
                  <div><p style={{ fontSize:14, fontWeight:700, color:'#c0392b', margin:0, lineHeight:1.4 }}>Station Commander — Always Category Leader</p><p style={{ fontSize:12, color:'#a8a29e', margin:'3px 0 0', fontWeight:500, lineHeight:1.4 }}>Officers in this category are automatically set as the category leader in the org chart.</p></div>
                </div>
              ):(
                <LeaderToggle value={!!form.isLeader} onChange={()=>setForm(p=>({...p,isLeader:!p.isLeader}))} />
              )}
              {form.isLeader&&!autoLeader&&!form.category&&<p style={{ fontSize:12, color:'#d97706', margin:'4px 0 0', fontWeight:600, display:'flex', alignItems:'center', gap:5, lineHeight:1.5 }}>⚠ Assign a category so the leader is placed correctly.</p>}
            </div>
          </form>
        </div>
        <div style={{ flexShrink:0, padding:'14px 24px 18px', borderTop:'1px solid #f0e8e5', background:'white', display:'flex', gap:10 }}>
          ...
          <button type="button" onClick={onClose} className="om-cancel-btn"
            style={{ flex:1, fontWeight:700, padding:'13px 0', borderRadius:12, fontSize:14, background:'#f5f0ed', border:'1px solid #e8ddd8', color:'#57534e', cursor:'pointer', transition:'all 0.15s', fontFamily:'inherit' }}>Cancel</button>
        </div>
      </div>
    </ModalPortal>
  );
    ...
    <div style={{ padding:'22px 18px 0', fontFamily:"'DM Sans', sans-serif", background:'#f5f3f0' }}>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', alignItems:'flex-end', justifyContent:'space-between', gap:14, marginBottom:18 }}>
        <div>
          ...
          <p style={{ fontSize:13, color:'#9a8a84', margin:'4px 0 0', lineHeight:1 }}>{officers.length} officer{officers.length!==1?'s':''} on record</p>
        </div>
        <button onClick={()=>setIsFormOpen(true)} className="om-add-btn"
          style={{ display:'flex', alignItems:'center', gap:7, fontSize:14, fontWeight:700, padding:'10px 20px', borderRadius:11, color:'white', border:'none', cursor:'pointer', background:'linear-gradient(135deg,#c0392b,#e67e22)', boxShadow:'0 4px 16px rgba(192,57,43,0.3)', transition:'all 0.18s', fontFamily:'inherit', flexShrink:0 }}>
          Add Officer
        </button>
      </div>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:7, marginBottom:13 }}>
        <button onClick={()=>setSelCat('All')} className="om-catbtn"
          style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', borderRadius:999, border:'none', background:selCat==='All'?'#1a1714':'white', boxShadow:selCat==='All'?'0 3px 12px rgba(28,25,23,0.22)':'0 1px 4px rgba(0,0,0,0.06)', outline:selCat==='All'?'none':'1px solid #e8e0dc', fontFamily:'inherit' }}>
          <Users size={11} style={{ color:selCat==='All'?'white':'#78716c' }} />
          ...
          <span style={{ fontSize:10, fontWeight:800, minWidth:17, height:15, padding:'0 5px', borderRadius:999, display:'flex', alignItems:'center', justifyContent:'center', background:selCat==='All'?'rgba(255,255,255,0.18)':'rgba(28,25,23,0.07)', color:selCat==='All'?'white':'#78716c' }}>{officers.length}</span>
        </button>
        {CATEGORY_OPTIONS.map(cat=>{
          ...
          return (
            <button key={cat} onClick={()=>setSelCat(on?'All':cat)} className="om-catbtn"
              style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', borderRadius:999, border:'none', background:on?c.dot:'white', boxShadow:on?'0 3px 14px rgba(0,0,0,0.18)':'0 1px 4px rgba(0,0,0,0.06)', outline:on?'none':`1px solid ${c.border}`, fontFamily:'inherit' }}>
              <span style={{ fontSize:11, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.08em', color:on?'white':c.color }}>{c.short}</span>
              <span style={{ fontSize:10, fontWeight:800, minWidth:17, height:15, padding:'0 5px', borderRadius:999, display:'flex', alignItems:'center', justifyContent:'center', background:on?'rgba(255,255,255,0.22)':c.light, color:on?'white':c.dot }}>{cnt}</span>
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:10 }}>
        <div style={{ position:'relative', flex:1, minWidth:160 }}>
          <Search size={13} style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'#c0392b', pointerEvents:'none' }} />
          <input type="text" className="om-si" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, rank, role, contact…"
            style={{ width:'100%', background:'white', border:'1px solid #ece6e0', borderRadius:9, padding:'9px 14px 9px 35px', fontSize:13, color:'#1a1714', outline:'none', fontFamily:'inherit' }} />
        </div>
        <div style={{ position:'relative' }}>
          <button onClick={()=>setRankOpen(p=>!p)}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'9px 14px', borderRadius:9, fontSize:13, fontWeight:700, cursor:'pointer', minWidth:148, justifyContent:'space-between', background:selRank!=='All Ranks'?'rgba(192,57,43,0.07)':'white', border:`1px solid ${selRank!=='All Ranks'?'rgba(192,57,43,0.25)':'#ece6e0'}`, color:selRank!=='All Ranks'?'#c0392b':'#78716c', fontFamily:'inherit' }}>
            <span style={{ display:'flex', alignItems:'center', gap:5 }}><Filter size={11} />{selRank!=='All Ranks'?(RANK_ABBREVIATIONS[selRank]||selRank):'All Ranks'}</span>
            <ChevronDown size={11} style={{ transform:rankOpen?'rotate(180deg)':'none', transition:'transform 0.2s' }} />
          </button>
          {rankOpen && (
            <div className="om-fdd" style={{ position:'absolute', right:0, top:'calc(100% + 5px)', borderRadius:10, overflow:'hidden', zIndex:30, background:'white', border:'1px solid #ece6e0', boxShadow:'0 12px 30px rgba(0,0,0,0.1)', minWidth:210, padding:'4px 0' }}>
              {uRanks.map(rk=>(
                <button key={rk} onClick={()=>{ setSelRank(rk); setRankOpen(false); }}
                  style={{ width:'100%', textAlign:'left', padding:'8px 14px', fontSize:13, fontWeight:selRank===rk?700:400, color:selRank===rk?'#c0392b':'#44403c', background:selRank===rk?'rgba(192,57,43,0.06)':'none', border:'none', cursor:'pointer', fontFamily:'inherit' }}
      ...
      {(search||selRank!=='All Ranks'||selCat!=='All')&&!loading&&(
        <p style={{ marginBottom:8, fontSize:12, fontWeight:600, color:'#a8a29e', lineHeight:1 }}>
      ...
      <div style={{ borderRadius:14, overflow:'hidden', background:'white', border:'1px solid #ece6e0', boxShadow:'0 2px 14px rgba(0,0,0,0.05)' }}>
        {loading ? (
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'64px 0', textAlign:'center' }}>
            ...
            <p style={{ fontSize:12, color:'#c4b5b0', margin:'4px 0 0' }}>{officers.length===0?'Click "Add Officer" to get started':'Try adjusting your filters'}</p>
          </div>
        ) : (
            ...
            <table className="om-table">
              ...
              <thead>
                <tr style={{ borderBottom:'1.5px solid #f5ede9' }}>
                  <th style={{ paddingLeft:20 }}>Officer</th>
                  ...
                  <th>Account No.</th>
                  <th>Birthdate</th>
                  <th className="c-right" style={{ paddingRight:16 }}></th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((o) => {
                    ...
                    <tr key={o.id} className="om-row-hover"
                      ...
                      <td style={{ paddingLeft:20, verticalAlign:'middle' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                          <Avatar src={o.profileImage} name={o.fullName} size={40} sx={{ border:'1.5px solid #f0e8e5', flexShrink:0 }} />
                          <div>
                            <p className="om-name-cell">{o.fullName}</p>
                            <p className="om-rank-sub">{rankAbbr}</p>
                          </div>
                        </div>
                      </td>
                      <td className="c-ctr" style={{ verticalAlign:'middle' }}>
                        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                          <span className="om-badge" style={{ background:'rgba(192,57,43,0.07)', border:'1px solid rgba(192,57,43,0.14)', color:'#c0392b' }}>
                            <Shield size={9} />{rankAbbr}
                          </span>
                          <span style={{ fontSize:11, color:'#c4b5b0', lineHeight:1.2, whiteSpace:'nowrap' }}>{o.rank}</span>
                        </div>
                      </td>
                      <td className="c-ctr" style={{ verticalAlign:'middle' }}>
                        {o.category && cc ? (
                          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                            ...
                            <span style={{ fontSize:11, color:'#c4b5b0', lineHeight:1.2, whiteSpace:'nowrap' }}>{cc.desc}</span>
                          </div>
                        ) : <span style={{ fontSize:13, color:'#d6ccc8' }}>—</span>}
                      </td>
                      <td className="c-ctr" style={{ verticalAlign:'middle' }}>
                        {isLdr ? (
                      ...
                      <td style={{ verticalAlign: exp?'top':'middle', alignItems:'center', paddingTop:14 }}>
                        {exp ? (
                          <>
                            <div style={{ display:'flex', flexDirection:'column', gap:3, marginBottom:4 }}>
                              {roles.map((r,i) => (
                                <span key={i} style={{ fontSize:13, fontWeight:500, color:'#44403c', lineHeight:1.5, display:'block' }}>{r}</span>
                              ))}
                            </div>
                          ...
                          <>
                            <span style={{ fontSize:13, fontWeight:500, color:'#44403c', whiteSpace:'nowrap', display:'block', lineHeight:1.5 }}>{roles[0]||'—'}</span>
                            {hasMore&&<button className="om-more-btn" onClick={()=>setExpanded(o.id)}> more</button>}
                          </>
                        )}
                      </td>
                      <td style={{ verticalAlign:'middle' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          ...
                          <span style={{ fontSize:13, color:'#57534e', whiteSpace:'nowrap' }}>{o.contactNumber||'—'}</span>
                        </div>
                      </td>
                      <td style={{ verticalAlign:'middle' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          <CreditCard size={11} style={{ color:'#d1c4be', flexShrink:0 }} />
                          <span style={{ fontSize:13, color:'#78716c', fontFamily:'Poppins', whiteSpace:'nowrap' }}>{o.accountNumber||'—'}</span>
                        </div>
                      </td>
                      <td style={{ verticalAlign:'middle' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          <Calendar size={11} style={{ color:'#d1c4be', flexShrink:0 }} />
                          <span style={{ fontSize:13, color:'#78716c', whiteSpace:'nowrap' }}>{fmtDate(o.birthdate)}</span>
                        </div>
                      </td>
