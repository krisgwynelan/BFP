...
import {
  ...
  ShieldCheck, HeartPulse, FileText, ClipboardCheck, Crown,
} from 'lucide-react';
...
const CAT = {
  ...
  'EMS': {
    Icon: HeartPulse,
    label: 'Emergency Medical Services', desc: 'Medical Response Unit', short: 'EMS',
    color: '#0f4c2a',
  ...
  'ADMIN': {
    Icon: FileText,
    label: 'Administrative Section', desc: 'Administrative Support', short: 'ADM',
    color: '#1e3a8a',
  ...
  'INSPECTOR': {
    Icon: ClipboardCheck,
    label: 'Fire Safety Enforcement Section', desc: 'Safety & Compliance', short: 'INSP',
    color: '#7c4a0a',
...
const FALLBACK = {
  Icon: Users,
  label: 'Operations Section', desc: 'Operations', short: '—',
  color: '#57534e',
...
// ─── Avatar ───────────────────────────────────────────────────────────────────
function Avatar({ src, name, size, iconComponent: IconComp }) {
  const [broken, setBroken] = useState(false);
  useEffect(() => { setBroken(false); }, [src]);

  if (!src || broken) {
    const FallbackIcon = IconComp || Users;
    ...
    return (
      <div style={{
        ...
        <FallbackIcon size={iconSize} style={{ color: 'rgba(255,255,255,0.75)', flexShrink: 0 }} strokeWidth={2} />
      </div>
    );
  }
  ...
  return (
    <img
      ...
      style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', objectPosition: 'center top', display: 'block', flexShrink: 0 }}
    />
  );
}

// ─── AvatarRing ───────────────────────────────────────────────────────────────
function AvatarRing({ src, name, ringSize, ring, gap, strip, glow, iconComponent }) {
  const imgSize = ringSize - 2 * ring - 2 * gap;
  return (
    <div style={{ width: ringSize, height: ringSize, borderRadius: '50%', background: strip, boxShadow: glow ? `0 4px 18px ${glow}` : 'none', padding: ring, flexShrink: 0, boxSizing: 'border-box' }}>
      <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: src ? 'white' : strip, padding: src ? gap : 0, boxSizing: 'border-box', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Avatar src={src} name={name} size={src ? imgSize : ringSize - 2 * ring} iconComponent={iconComponent} />
      </div>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        width: CARD_W, minWidth: CARD_MINW, borderRadius: CARD_RADIUS, overflow: 'visible',
        background: 'white', border: `${borderWidth}px solid ${borderColor}`,
        boxShadow: hovered ? `0 16px 40px ${shadowBase}, 0 0 0 3px ${borderColor}40` : isLeader ? `0 6px 22px ${shadowBase}` : `0 2px 10px ${shadowBase}`,
        transform: hovered ? 'translateY(-6px)' : 'translateY(0)',
        transition: 'transform 0.22s ease, box-shadow 0.22s ease',
      }}
    >
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: `${isLeader ? 20 : 16}px 18px 18px`, flex: 1, width: '100%' }}>
        <div style={{ position: 'relative', marginBottom: 18, flexShrink: 0 }}>
          <AvatarRing src={officer.profileImage} name={officer.fullName} ringSize={RING_OUTER} ring={ringPx} gap={GAP_PX} strip={c.strip} glow={isLeader ? c.leaderGlow : null} iconComponent={Icon} />
          <div style={{ position: 'absolute', bottom: -11, left: '50%', transform: 'translateX(-50%)', background: c.bandBg, color: 'white', fontSize: 10, fontWeight: 800, padding: '3px 9px', borderRadius: 6, letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap', border: '2px solid white', boxShadow: '0 1px 4px rgba(0,0,0,0.18)', lineHeight: 1.4 }}>{abbr(officer.rank)}</div>
        </div>
        <p style={{ fontSize: isLeader ? 17 : 16, fontWeight: isLeader ? 800 : 700, color: '#111827', textAlign: 'center', margin: '0 0 3px', lineHeight: 1.3, width: '100%', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-word' }}>{officer.fullName}</p>
        <p style={{ fontSize: 11.5, fontWeight: 500, color: c.pillColor, textAlign: 'center', margin: '0 0 10px', width: '100%', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{officer.rank}</p>
        <div style={{ width: '100%', height: 1, background: `linear-gradient(to right,transparent,${c.border},transparent)`, margin: '0 0 10px', flexShrink: 0 }} />
        {role && <p style={{ fontSize: 11.5, fontWeight: 500, color: '#78716c', textAlign: 'center', margin: '0', width: '100%', lineHeight: 1.45, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-word' }}>{role}</p>}
        {officer.contactNumber && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 'auto', paddingTop: 9, maxWidth: '100%' }}>
            ...
            <span style={{ fontSize: 11.5, color: '#b0a8a4', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{officer.contactNumber}</span>
          </div>
        )}
      </div>
    </div>
...
function OfficerCard({ officer }) {
  ...
  return (
    <div
      ...
      style={{
        position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center',
        background: 'white', border: `${officer.isLeader ? 2 : 1.5}px solid ${officer.isLeader ? c.leaderBorder : c.border}`,
        borderRadius: CARD_RADIUS, overflow: 'visible',
        boxShadow: hovered ? `0 14px 36px ${c.leaderGlow}, 0 0 0 3px ${(officer.isLeader ? c.leaderBorder : c.border)}40` : officer.isLeader ? `0 4px 16px ${c.leaderGlow}` : `0 2px 8px ${c.glow}`,
        transform: hovered ? 'translateY(-5px)' : 'translateY(0)',
        transition: 'transform 0.2s, box-shadow 0.2s',
      }}
    >
      <div style={{ width: '100%', height: officer.isLeader ? STRIP_H_LEADER : STRIP_H_MEMBER, background: c.strip, borderRadius: `${CARD_RADIUS}px ${CARD_RADIUS}px 0 0`, flexShrink: 0 }} />
      {officer.isLeader && (
        <div style={{ position: 'absolute', top: -(STRIP_H_LEADER + 12), left: '50%', transform: 'translateX(-50%)', zIndex: 10, display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 11px', borderRadius: 20, background: c.strip, border: '2.5px solid white', boxShadow: `0 3px 10px ${c.leaderGlow}`, whiteSpace: 'nowrap', pointerEvents: 'none' }}>
          <Crown size={10} color="white" strokeWidth={2.6} />
          <span style={{ fontSize: 9, fontWeight: 900, color: 'white', letterSpacing: '0.16em', textTransform: 'uppercase' }}>Category Leader</span>
        </div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: `${officer.isLeader ? 20 : 16}px 14px 16px`, flex: 1, width: '100%' }}>
        <div style={{ position: 'relative', marginBottom: 18, flexShrink: 0 }}>
          <AvatarRing src={officer.profileImage} name={officer.fullName} ringSize={RING_OUTER} ring={ringPx} gap={GAP_PX} strip={c.strip} glow={officer.isLeader ? c.leaderGlow : null} iconComponent={Icon} />
          <div style={{ position: 'absolute', bottom: -11, left: '50%', transform: 'translateX(-50%)', background: c.bandBg, color: 'white', fontSize: 10, fontWeight: 800, padding: '3px 9px', borderRadius: 6, letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap', border: '2px solid white', boxShadow: '0 1px 4px rgba(0,0,0,0.18)', lineHeight: 1.4 }}>{abbr(officer.rank)}</div>
        </div>
        <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', textAlign: 'center', margin: '0 0 3px', lineHeight: 1.3, width: '100%', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordBreak: 'break-word' }}>{officer.fullName}</p>
        <p style={{ fontSize: 11, fontWeight: 500, color: c.pillColor, textAlign: 'center', margin: '0 0 9px', width: '100%', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{officer.rank}</p>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px 3px 7px', borderRadius: 6, background: c.pillBg, border: `1px solid ${c.border}`, fontSize: 9.5, fontWeight: 800, color: c.pillColor, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: role ? 6 : 0, flexShrink: 0 }}>
          <Icon size={9} strokeWidth={2.4} />{c.label}
        </span>
        {role && <p style={{ fontSize: 11.5, fontWeight: 500, color: '#78716c', textAlign: 'center', margin: '0', width: '100%', lineHeight: 1.4, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{role}</p>}
        {officer.contactNumber && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 'auto', paddingTop: 9, maxWidth: '100%' }}>
            <Phone size={10} style={{ color: '#c4b5b0', flexShrink: 0 }} />
            <span style={{ fontSize: 11.5, color: '#b0a8a4', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{officer.contactNumber}</span>
...
function OrgSection({ cat, officers }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', animation: 'opFU 0.24s ease both' }}>
      <div className="op-section-label" style={{ display: 'inline-flex', alignItems: 'center', gap: 12, padding: '10px 20px 10px 12px', borderRadius: 14, background: c.bg, border: `1.5px solid ${c.border}`, boxShadow: `0 4px 16px ${c.glow}`, marginBottom: 20 }}>
        <div style={{ width: 38, height: 38, borderRadius: 10, background: c.strip, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div className="op-section-label-text" style={{ lineHeight: 1.25 }}>
          ...
          <p style={{ fontSize: 10.5, fontWeight: 500, color: c.pillColor, margin: 0, textAlign: 'center' }}>{c.desc}</p>
        </div>
        <span style={{ fontSize: 11, fontWeight: 800, padding: '2px 9px', borderRadius: 6, background: c.pillBg, color: c.pillColor, border: `1px solid ${c.border}`, marginLeft: 2 }}>{officers.length}</span>
      </div>
      ...
      {leader ? (
        <>
          ...
          {members.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '16px 0 8px' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 13px', borderRadius: 7, background: c.pillBg, border: `1px solid ${c.border}` }}>
                <Users size={10} color={c.pillColor} strokeWidth={2.4} />
                <span style={{ fontSize: 9.5, fontWeight: 800, color: c.pillColor, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Personnel · {members.length}</span>
              </div>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 9, background: c.pillBg, border: `1.5px dashed ${c.border}`, marginBottom: 16 }}>
...
export function OfficersPage() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: '#faf8f7', minHeight: '100vh' }}>
      ...
      {/* ── HERO ── */}
      <section className="op-hero-pad" style={{ position: 'relative', overflow: 'hidden', padding: '68px 20px 76px', background: 'linear-gradient(150deg,#7f1d1d 0%,#991b1b 38%,#c0392b 72%,#d35400 100%)' }}>
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', opacity: 0.06, backgroundImage: 'linear-gradient(rgba(255,255,255,0.5) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.5) 1px,transparent 1px)', backgroundSize: '48px 48px' }} />
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <div className="op-hero-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 8, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', marginBottom: 20 }}>
            <Users size={13} color="rgba(255,255,255,0.8)" strokeWidth={2} />
            <span style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.24em', color: 'rgba(255,255,255,0.8)' }}>BFP Station 1 · Station Personnel</span>
          </div>
          <h1 className="op-hero-h1" style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(3rem,8vw,5.5rem)', letterSpacing: '0.04em', color: 'white', lineHeight: 1, margin: '0 0 14px' }}>Our Officers</h1>
          ...
          <div className="op-hero-stats" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10 }}>
            {[{ v: loading ? '…' : sorted.length, l: 'Active Officers' }, { v: '24 / 7', l: 'On Duty' }, { v: 'Station 1', l: 'Assignment' }].map(({ v, l }) => (
      ...
      <section className="op-filters-section" style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px 0' }}>
        <div className="op-filter-row" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
            ...
            <input type="text" className="op-si" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, rank, or assignment…" style={{ width: '100%', paddingLeft: 40, paddingRight: 14, paddingTop: 11, paddingBottom: 11, borderRadius: 10, fontSize: 13, fontWeight: 500, background: 'white', border: '1.5px solid #e8e2de', color: '#1c1917', fontFamily: "'DM Sans',sans-serif", transition: 'border-color 0.18s,box-shadow 0.18s', outline: 'none' }} />
          </div>
          <div style={{ position: 'relative' }}>
            <button onClick={() => setRankOpen(p => !p)} className="op-rank-btn" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 16px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', minWidth: 168, justifyContent: 'space-between', background: selRank !== 'All Ranks' ? 'rgba(192,57,43,0.06)' : 'white', border: `1.5px solid ${selRank !== 'All Ranks' ? 'rgba(192,57,43,0.25)' : '#e8e2de'}`, color: selRank !== 'All Ranks' ? '#c0392b' : '#78716c', fontFamily: "'DM Sans',sans-serif" }}>
            ...
            {rankOpen && (
              <div className="op-fdd" style={{ position: 'absolute', right: 0, top: '100%', marginTop: 6, borderRadius: 10, overflow: 'hidden', zIndex: 30, background: 'white', border: '1.5px solid #ede8e5', boxShadow: '0 8px 28px rgba(0,0,0,0.09)', minWidth: 220, padding: '4px 0' }}>
                {uRanks.map(rk => <button key={rk} className={`op-ro ${selRank === rk ? 'act' : ''}`} onClick={() => { setSelRank(rk); setRankOpen(false); }} style={{ color: selRank === rk ? '#c0392b' : '#44403c' }}>{rk}</button>)}
              </div>
            )}
          </div>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
          <button className="op-tab" onClick={() => setSelCat('All')} style={selCat === 'All' ? { background: 'linear-gradient(135deg,#c0392b,#e67e22)', color: 'white', border: '1.5px solid transparent', boxShadow: '0 3px 12px rgba(192,57,43,0.25)', transform: 'translateY(-1px)' } : { color: '#78716c' }}>
      ...
      <section className="op-officers-section" style={{ maxWidth: 1400, margin: '0 auto', padding: '28px 20px 60px' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {/* Root node */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div className="op-root-node" style={{ display: 'inline-flex', alignItems: 'center', gap: 14, padding: '15px 30px', borderRadius: 18, background: 'linear-gradient(135deg,#7f1d1d,#c0392b)', boxShadow: '0 8px 32px rgba(127,29,29,0.32)' }}>
                ...
                <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 40 }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '10px 18px 10px 12px', borderRadius: 13, background: '#f5f4f3', border: '1.5px solid #ddd5d0', boxShadow: '0 4px 12px rgba(0,0,0,0.06)', marginBottom: 22 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 9, background: 'linear-gradient(135deg,#78716c,#a8a29e)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Users size={17} color="white" strokeWidth={2.2} />
                    </div>
                    <div>
